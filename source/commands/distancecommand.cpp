...
vector<string> DistanceCommand::setParameters(){	
	try {
		...
		CommandParameter pcalc("calc", "Multiple", "nogaps-eachgap-onegap-jtt-pmb-pam", "onegap", "", "", "","",false,false); parameters.push_back(pcalc);
...
string DistanceCommand::getHelpString(){	
	try {
		...
		helpString += "The calc parameter allows you to specify the method of calculating the distances.  Your options are: nogaps, onegap or eachgap for dna/rna sequences. If using protein sequences, your calc options are jtt, pmb, pam....... The default is onegap.\n";
...
DistanceCommand::DistanceCommand(string option) {
	try {
		...
		if(option == "help") { help(); abort = true; calledHelp = true; }
            ...
            if ((calc != "onegap") && (calc != "eachgap") && (calc != "nogaps") && (calc != "jtt") && (calc != "pmb") && (calc != "pam")) { m->mothurOut(calc + " is not a valid calc. Options are eachgap, onegap, nogaps, jtt, pmb, pam. I'll use onegap.\n");  calc = "onegap";  }
            
            prot = false; //not using protein sequences
            if ((calc == "jtt") || (calc == "pmb") || (calc == "pam")) { prot = true; }
