...
int MakeContigsCommand::execute(){
	try {
        ...
        out << "Name\tLength\tOverlap_Length\tOverlap_Start\tOverlap_End\tMisMatches\tNum_Ns\n"; out.close();//print Headers
...
struct contigsData {
    ...
    char delim;
    int nameType, offByOneTrimLength, pdiffs, bdiffs, tdiffs, kmerSize, insert, deltaq;
...
int convertProb(double qProb){
    try {
        vector<double> qual_score; qual_score.resize(47);
        ...
        while (good) {
            ...
            if (!ignore) {
                ...
                if(trashCode.length() == 0){
                    ...
                    if(!ignore){
                        //output
                        string output = ">" + fSeq.getName() + '\t' + commentString + "\n" + contig + "\n";
                        ...
                        output = fSeq.getName() + '\t' + toString(contig.length()) + '\t' + toString(oend-oStart) + '\t' + toString(oStart) + '\t' + toString(oend) + '\t' + toString(numMismatches) + '\t' + toString(numNs) + "\n";
                        params->misMatchesFile->write(output);
                    }
                }else{
                    params->badNames.insert(fSeq.getName());
                    
                    string output = ">" + fSeq.getName() + " | " + trashCode + '\t' + commentString + "\n" + contig + "\n";
...
unsigned long long MakeContigsCommand::createProcesses(vector<string> fileInputs, vector<string> qualOrIndexFiles, string outputFasta, string outputScrapFasta, string outputQual, string outputScrapQual, string outputMisMatches, vector<vector<string> > fastaFileNames, vector<vector<string> > qualFileNames, string group, map<int, oligosPair>& pairedPrimers, map<int, oligosPair>& rpairedPrimers, map<int, oligosPair>& pairedBarcodes, map<int, oligosPair>& rpairedBarcodes, vector<string>& barcodeNames, vector<string>& primerNames) {
    try {
        ...
        for (int i = 0; i < processors-1; i++) {
            ...
            dataBundle->setVariables(gz, delim, nameType, offByOneTrimLength, pairedBarcodes, pairedPrimers, rpairedBarcodes, rpairedPrimers, primerNames, barcodeNames, reorient, pdiffs, bdiffs, tdiffs, align, match, misMatch, gapOpen, gapExtend, insert, deltaq, kmerSize, format, trimOverlap, createOligosGroup, group);
...
unsigned long long MakeContigsCommand::createProcessesGroups(vector< vector<string> > fileInputs, string compositeFastaFile, string compositeScrapFastaFile, string compositeQualFile, string compositeScrapQualFile, string compositeMisMatchFile, map<int, string>& file2Groups) {
    try {
        ...
        for (int i = 0; i < processors-1; i++) {
            ...
            dataBundle->setVariables(gz, delim, nameType, offByOneTrimLength, pairedBarcodes, pairedPrimers, rpairedBarcodes, rpairedPrimers, primerNames, barcodeNames, reorient, pdiffs, bdiffs, tdiffs, align, match, misMatch, gapOpen, gapExtend, insert, deltaq, kmerSize, format, trimOverlap, createOligosGroup, "");
