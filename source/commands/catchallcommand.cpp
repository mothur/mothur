...
int CatchAllCommand::createSummaryFile(string file1, string label, ofstream& out) {
	try {
		...
		if (!able) {  m->mothurOut("[ERROR]: the catchall program did not run properly. Please check to make sure it is located in the same folder as your mothur executable.");m->mothurOutEndLine();  m->control_pressed = true; return 0; }
