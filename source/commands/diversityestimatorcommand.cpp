...
vector<string> EstimatorSingleCommand::setParameters(){
    try {
        ...
        CommandParameter pcalc("calc", "Multiple", "erarefact-metroig-metroln-metrols-metrosichel-igabund-igrarefact-lnrarefact-lnabund-lnshift-lsabund-lsrarefact-siabund-sirarefact", "erarefact", "", "", "","",true,false,true); parameters.push_back(pcalc); //lnabund
...
string EstimatorSingleCommand::getHelpString(){
    try {
        ...
        helpString += "The burn parameter allows ignore part of the sampling file.  Default = 200000 / 100000 for IGAbundance, LNShift, LSAbundance / IGRarefaction, LNRarefaction, LSRarefaction, SIAbundance, SIRarefaction respectively.\n";
        helpString += "The burnsample parameter allows you to set sampling frequency.  The default is 1000 / 100 for IGAbundance, LNShift, LSAbundance / IGRarefaction, LNRarefaction, LSRarefaction, SIAbundance, SIRarefaction respectively.\n";
...
EstimatorSingleCommand::EstimatorSingleCommand(string option)  {
    try {
        ...
        if(option == "help") { help(); calledHelp = true; abort = true; }
            ...
            estimatorsThatRequireSampleFile.insert("igrarefact");
            estimatorsThatRequireSampleFile.insert("lnabund");
            estimatorsThatRequireSampleFile.insert("lnrarefact");
            ...
            estimatorsThatRequireSampleFile.insert("sirarefact");
            ...
            if ((util.isEqual(coverage, -1)) && ((util.inUsersGroups("igrarefact", Estimators)) || (util.inUsersGroups("lnrarefact", Estimators)) || (util.inUsersGroups("lsrarefact", Estimators)))) {
                m->mothurOut("[ERROR]: You must set the coverage parameter to run the igrarefact, lsrarefact or lnrarefact estimator. Aborting.\n"); abort=true;
            }
...
int EstimatorSingleCommand::process(SAbundVector*& sabund, string fileRoot) {
    try {
        ...
        for (int i = 0; i < Estimators.size(); i++) {
            ...
            if (Estimators[i] == "erarefact")            { runErarefaction(sabund, fileRoot);    }
            ...
            else if (Estimators[i] == "igrarefact")      { runIGRarefaction(sabund, fileRoot);   }
            else if (Estimators[i] == "lnabund")         { runLNAbund(sabund, fileRoot);         }
            else if (Estimators[i] == "lnrarefact")      { runLNRarefaction(sabund, fileRoot);   }
            ...
            else if (Estimators[i] == "sirarefact")      { runSIRarefaction(sabund, fileRoot);   }
        }
...
string EstimatorSingleCommand::runLNRarefaction(SAbundVector*& sabund, string fileRoot) {
    try {
        ...
        out << "LNRarefaction_Lower\tLNRarefaction_Median\tLNRarefaction_Upper\n";
...
//**********************************************************************************************************************
string EstimatorSingleCommand::runLSRarefaction(SAbundVector*& sabund, string fileRoot) {
    try {
        map<string, string> variables;
        variables["[filename]"] = fileRoot;
        variables["[distance]"] = sabund->getLabel();
        string outputFileName = getOutputFileName("lsrarefaction", variables);
        outputNames.push_back(outputFileName); outputTypes["lsrarefaction"].push_back(outputFileName);
        
        ofstream out; util.openOutputFile(outputFileName, out); //format output
        out.setf(ios::fixed, ios::floatfield); out.setf(ios::showpoint);
        
        out << "LSRarefaction_Lower\tLSRarefaction_Median\tLSRarefaction_Upper\n";
        
        LSRarefaction lsRare(coverage);
        ...
        m->errorOut(e, "EstimatorSingleCommand", "runLSRarefaction");
        exit(1);
    }
}
//**********************************************************************************************************************
string EstimatorSingleCommand::runSIRarefaction(SAbundVector*& sabund, string fileRoot) {
    try {
        map<string, string> variables;
        variables["[filename]"] = fileRoot;
        variables["[distance]"] = sabund->getLabel();
        string outputFileName = getOutputFileName("sirarefaction", variables);
        outputNames.push_back(outputFileName); outputTypes["sirarefaction"].push_back(outputFileName);
        
        ofstream out; util.openOutputFile(outputFileName, out); //format output
        out.setf(ios::fixed, ios::floatfield); out.setf(ios::showpoint);
        
        out << "SIRarefaction_Lower\tSIRarefaction_Median\tSIRarefaction_Upper\n";
        
        SIRarefaction siRare(coverage);
        ...
        m->errorOut(e, "EstimatorSingleCommand", "runSIRarefaction");
        exit(1);
    }
}
//**********************************************************************************************************************
