...
struct vsearchData {
    ...
    string groupfile;
    string outputFName;
    string accnos, alns, filename, templatefile, vsearchLocation, countlist;
    MothurOut* m;
    int start;
    int end;
    int threadID, count, numChimeras;
    vector<string> groups;
    bool dups, useAbskew, chimealns, useMinH, useMindiv, useXn, useDn, useMindiffs, hasCount;
    string abskew, minh, mindiv, xn, dn, mindiffs;
    
    vsearchData(){}
    vsearchData(string o, string vloc, string t, string file, string f, string n, string g, string ac,  string al, string nc, vector<string> gr, MothurOut* mout, int st, int en, int tid) {
        fastafile = f;
        namefile = n;
        groupfile = g;
        filename = file;
        ...
        count = 0;
        numChimeras = 0;
        vsearchLocation = vloc;
        countlist = nc;
    }
    void setBooleans(bool dps, bool Abskew, bool calns, bool MinH, bool Mindiv, bool Xn, bool Dn, bool umd, bool hc) {
        useAbskew = Abskew;
        chimealns = calns;
        useMinH = MinH;
        useMindiv = Mindiv;
    ...
    void setVariables(string abske, string min, string mindi, string x, string d, string md) {
        abskew = abske;
        minh = min;
        mindiv = mindi;
        xn = x;
