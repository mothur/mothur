...
int SplitMatrix::splitDistance(){
	try {
        ...
        vector<map<string, string > > tempDistFiles;
        for(int i=0;i<numGroups;i++){
            string inputFileName = (inputFile + "." + toString(i) + ".temp");
            util.mothurRemove(inputFileName);
            
            bool addFilePair = true;
            ...
            if (addFilePair) {
                map<string, string > temp;
                temp[groupFiles[i]] = inputFileName;
                dists.push_back(temp);
                
                if (countfile != "") {
                    //add header
                    ofstream out; util.openOutputFile(inputFileName, out);
                    out << "Representative_Sequence\ttotal" << endl; out.close();
                }
            }
        }

        singleton = inputFile + ".extra.temp";
        ofstream remainingNames; util.openOutputFile(singleton, remainingNames);
        if (countfile != "") {  remainingNames << "Representative_Sequence\ttotal" << endl; }
        ...
        if (countfile != "") {
            ...
            for (map<string, int>::iterator itCtMap = ctMap.begin(); itCtMap != ctMap.end(); itCtMap++) {
                string name = itCtMap->first;
                int abundance = itCtMap->second;
                int seqIndex = nameAssignment[name];
    
                if (seqsGroupAssignment[seqIndex] != -1) {
                    int groupID = findRootGroup(mergedGroups, seqsGroupAssignment[seqIndex]);
                    
                    ofstream outFile; util.openOutputFileAppend((inputFile + "." + toString(groupID) + ".temp"), outFile);
                    outFile << name << '\t' << abundance << endl;
                    outFile.close();
                }else{
                    wroteExtra = true;
            ...
            ifstream bigNameFile; util.openInputFile(inputFile, bigNameFile);
            ...
            while(!bigNameFile.eof()){
                ...
                if (seqsGroupAssignment[seqIndex] != -1) {
                    ...
                    ofstream outFile; util.openOutputFileAppend((inputFile + "." + toString(groupID) + ".temp"), outFile);                    outFile << name << '\t' << nameList << endl;
                    outFile.close();
                }else{
                    wroteExtra = true;
                    remainingNames << name << '\t' << nameList << endl;
